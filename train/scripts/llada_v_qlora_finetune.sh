...
SAVE_STRATEGY=${SAVE_STRATEGY:-"steps"}   # "steps" or "epoch"
SAVE_INTERVAL=${SAVE_INTERVAL:-2000}      # steps if SAVE_STRATEGY=steps, ignored otherwise

# TODO: set num_node / gpu_num, and fill DATA_PATH / IMAGE_FOLDER / VIDEO_FOLDER.
...
VIDEO_FOLDER=""          # leave empty if not using video

PROMPT_VERSION="llava_llada"
BASE_RUN_NAME="llada_v_qlora"
echo "BASE_RUN_NAME: ${BASE_RUN_NAME}"

# QLoRA-friendly launch (4-bit + LoRA adapters). For single-GPU, run without DeepSpeed to avoid full fp16 init.
ACCELERATE_CPU_AFFINITY=0 torchrun --nproc_per_node=${gpu_num} --nnodes=${num_node} --master_addr=${MASTER_ADDR} --master_port ${MASTER_PORT} --node_rank=${RANK} \
